...
1. **先讀 [CONTRIBUTING.md](CONTRIBUTING.md) 的「四個鐵律」**（必須遵守，未來任何在這 repo 的工作都套用）：
   - 鐵律 1：AI 加工清潔原則（過程文件不入 repo）
   - 鐵律 2：階段性備份紀律（每 phase commit + push，不堆積）
   - 鐵律 3：Bug 報告先問「上次正常是什麼時候」
   - 鐵律 4：Git history 不可刪改（push 過的 commit 永久保留）
2. **依 Step 0 → Step 9 順序執行**，每步用對應驗證命令確認結果再往下
3. **遇到 ❌ 路徑** → 停下來，把錯誤訊息丟給使用者讓他看，問下一步怎麼處理
4. **指令執行前** → 先用簡短一句話告訴使用者你要做什麼
5. **API key / Tailscale account 等需要瀏覽器操作** → 不要嘗試自動化，明確告訴使用者「請開瀏覽器去 X」並等他確認完成
6. **寫入任何檔案前** → 確認路徑正確，避免覆蓋使用者既有檔案（如 `~/.claude/settings.json` 已有 hooks 配置）
7. **進度報告** → 每完成一步打勾並簡短說明下一步
